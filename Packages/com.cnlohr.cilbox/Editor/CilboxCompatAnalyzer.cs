	...
	public static class CilboxCompatAnalyzer
	{
		...
		public static void Analyze( bool verbose )
		{
			...
			foreach( OpcodeIssue oi in opcodeIssues.Values )
			{
				...
				CilboxClickableFaults.LogSourceException( msg,
					new List<CilboxClickableFaults.WorldFrame> { new CilboxClickableFaults.WorldFrame( oi.typeFullName, oi.method, oi.firstIlOffset ) } );
			}
			...
			foreach( GapIssue g in gapIssues.Values )
			{
				...
				CilboxClickableFaults.LogSourceException(
					$"[Cilbox compat] {g.message}, in {g.typeDisplay}.{g.method}" + ( g.count > 1 ? $" (x{g.count})" : "" ) + " -- throws at runtime.",
					new List<CilboxClickableFaults.WorldFrame> { new CilboxClickableFaults.WorldFrame( g.typeFullName, g.method, g.ilOffset ) } );
			}
			...
			{
				if( reported++ > kMaxReported ) break;
				CilboxClickableFaults.LogSourceExceptionLines( "[Cilbox compat] Cilbox whitelist violation: " + wl.Key, wl.Value, false );
			}
			foreach( string fw in framelessWl )
			{
				if( reported++ > kMaxReported ) break;
				CilboxClickableFaults.LogSourceExceptionLines( "[Cilbox compat] Cilbox whitelist violation: " + fw, null, true );
			}

			foreach( KeyValuePair<string, string> p in policeIssues )
			{
				if( reported++ > kMaxReported ) break;
				CilboxClickableFaults.LogSourceExceptionLines( "[Cilbox compat] Basis content-police: " + p.Key,
		...
		static void AddUsageLines( Dictionary<string, List<Site>> memberSites, string key, List<string> lines, HashSet<string> affectedScripts )
		{
			...
			var frames = new List<CilboxClickableFaults.WorldFrame>();
			var seen = new HashSet<string>();
			foreach( Site s in sites )
			{
				...
				if( frames.Count < 20 )
					frames.Add( new CilboxClickableFaults.WorldFrame( s.typeFullName, s.method, s.ilOffset ) );
			}
			lines.AddRange( CilboxClickableFaults.ResolveFrameLines( frames ) );
		}
		...
		static string ScriptFrameLine( Component comp, string typeFullName )
		{
			...
			return CilboxClickableFaults.FrameLineForType( typeFullName, path, FindTypeDeclarationLine( path, simple ) );
		}
